...
class TablePlanner {
    ...
    */
   public TablePlanner(String tblname, Predicate mypred, Transaction tx, int joinType) {
      this.mypred  = mypred;
      this.tx  = tx;
   ...
   public Plan makeJoinPlan(Plan current) {
      ...
      case 1:
    	  ...
    	  break;  
      case 2:
         System.out.println("BLOCK NESTED LOOP JOIN");
    	  p = makeBlockNestedJoin(current, currsch);
    	  break;
      }
   ...
   private Plan makeNestedJoin(Plan current, Schema currsch) {
      for (String fldname : currsch.fields()) {
         String outerfield = mypred.equatesWithField(fldname);
         if (outerfield != null && myschema.hasField(outerfield)) {
            Plan p = new NestedLoopJoinPlan(current, myplan, fldname, outerfield, tx);
            p = addSelectPred(p);
            return addJoinPred(p, currsch);
         }
      }
